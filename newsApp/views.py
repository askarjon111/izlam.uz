    ...
    if request.POST:
        ...
        if user is not None:
            if user.is_active:
                ...
                resp['msg'] = "Noto'g'ri foydalanuvchi nomi yoki parol"
        else:
            resp['msg'] = "Noto'g'ri foydalanuvchi nomi yoki parol"
...
def update_profile(request):
    context = context_data()
    context['page_title'] = 'Profilni sozlash'
    ...
    if not request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Profil o'zgardi")
...
def update_password(request):
    context = context_data()
    context['page_title'] = "Parolni o'zgartirish"
...
def manage_post(request, pk = None):
    ...
    if not pk is None:
        ...
        context['page_title']="Maqolani o'zgartirish"
        context['post']=models.Post.objects.get(id=pk)
    else:
        context['page']='new_post'
        context['page_title']='Yangi maqola'
        context['post']={}
...
def save_post(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            resp['id'] = postID
            resp['status'] = 'success'
            messages.success(request, "Post muvaffaqiyatli saqlandi.")
        else:
            for field in form:
...
def list_posts(request):
    ...
    context['page_title'] = 'Barcha maqolalar'
